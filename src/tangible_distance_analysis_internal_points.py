...
Calculate the distance matrix between a set of NeuronMorphology *INTERNAL* points via WBC
tangible download
...
For each morphology, get all the internal points.  
For each internal point, apply the position / rotation of the tangible. 
For each internal point in one morphology, search to find the nearest 
...
def distance_between_intpoints( f1, f2 ):
    ...
    # forest
    matrix1 = getInternalPoints(f1)
    matrix2 = getInternalPoints(f2)
...
for f1 in forests:
    for f2 in forests:
        min_distance_complete_set.append(distance_between_intpoints(f1, f2))
